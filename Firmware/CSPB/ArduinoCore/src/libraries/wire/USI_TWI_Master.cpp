...
unsigned char TinyM_USI_TWI_Start_Transceiver_With_Data( unsigned char * , unsigned char );
unsigned char TinyM_USI_TWI_Master_Transfer( unsigned char );
unsigned char TinyM_USI_TWI_Master_Start( void );

union  TinyM_USI_TWI_state
{
...
---------------------------------------------------------------*/
unsigned char TinyM_USI_TWI_Get_State_Info( void )
{
  return ( TinyM_USI_TWI_state.errorState );                            // Return error state.
}
/*---------------------------------------------------------------
 ...
 USI_TWI_Master.h
---------------------------------------------------------------*/
unsigned char TinyM_USI_TWI_Start_Random_Read( unsigned char *msg, unsigned char msgSize)
{
  *(msg) &= ~(TRUE<<TWI_READ_BIT);		// clear the read bit if it's set
  TinyM_USI_TWI_state.errorState = 0;
  TinyM_USI_TWI_state.memReadMode = TRUE;
  
  return (TinyM_USI_TWI_Start_Transceiver_With_Data( msg, msgSize));
}
/*---------------------------------------------------------------
 ...
 USI_TWI_Master.h
---------------------------------------------------------------*/
unsigned char TinyM_USI_TWI_Start_Read_Write( unsigned char *msg, unsigned char msgSize)
{
    
	TinyM_USI_TWI_state.errorState = 0;				// Clears all mode bits also
...
/*---------------------------------------------------------------
 ...
 USI_TWI_Master.h
---------------------------------------------------------------*/
unsigned char TinyM_USI_TWI_Start_Transceiver_With_Data( unsigned char *msg, unsigned char msgSize)
{
...
//  TinyM_USI_TWI_state.errorState = 0;				// Clears all mode bits also
  ...
  TinyM_USI_TWI_state.addressMode = TRUE;			// Always true for first byte

#ifdef PARAM_VERIFICATION
  if(msg > (unsigned char*)RAMEND)                 // Test if address is outside SRAM space
  {
    TinyM_USI_TWI_state.errorState = USI_TWI_DATA_OUT_OF_BOUND;
    return (FALSE);
  }
  if(msgSize <= 1)                                 // Test if the transmission buffer is empty
  {
    TinyM_USI_TWI_state.errorState = USI_TWI_NO_DATA;
    return (FALSE);
  }
#endif
  ...
  if( USISR & (1<<USISIF) )
  {
    TinyM_USI_TWI_state.errorState = USI_TWI_UE_START_CON;
    return (FALSE);
  }
  if( USISR & (1<<USIPF) )
  {
    TinyM_USI_TWI_state.errorState = USI_TWI_UE_STOP_CON;
    return (FALSE);
  }
  if( USISR & (1<<USIDC) )
  {
    TinyM_USI_TWI_state.errorState = USI_TWI_UE_DATA_COL;
    return (FALSE);
  }
#endif
  ...
  {
    TinyM_USI_TWI_state.masterWriteDataMode = TRUE;
  }

//	if (TinyM_USI_TWI_state.memReadMode)
//	{
		savedMsg = msg;
		savedMsgSize = msgSize;
//	}

	if ( !TinyM_USI_TWI_Master_Start( ))
  {
...
/*Write address and Read/Write data */
  do
  {
    ...
    if (TinyM_USI_TWI_state.addressMode || TinyM_USI_TWI_state.masterWriteDataMode)
    {
      /* Write a byte */
      PORT_USI &= ~(1<<PIN_USI_SCL);                // Pull SCL LOW.
      USIDR     = *(msg++);                        // Setup data.
      TinyM_USI_TWI_Master_Transfer( tempUSISR_8bit );    // Send 8 bits on bus.
      
      /* Clock and verify (N)ACK from slave */
      DDR_USI  &= ~(1<<PIN_USI_SDA);                // Enable SDA as input.
      if( TinyM_USI_TWI_Master_Transfer( tempUSISR_1bit ) & (1<<TWI_NACK_BIT) ) 
      {
        if ( TinyM_USI_TWI_state.addressMode )
          TinyM_USI_TWI_state.errorState = USI_TWI_NO_ACK_ON_ADDRESS;
        else
          TinyM_USI_TWI_state.errorState = USI_TWI_NO_ACK_ON_DATA;
        return (FALSE);
      }
	  
	  if ((!TinyM_USI_TWI_state.addressMode) && TinyM_USI_TWI_state.memReadMode)// means memory start address has been written
	  {
		msg = savedMsg;					// start at slave address again
		*(msg) |= (TRUE<<TWI_READ_BIT);  // set the Read Bit on Slave address
		TinyM_USI_TWI_state.errorState = 0;
		TinyM_USI_TWI_state.addressMode = TRUE;	// Now set up for the Read cycle
		...
		if ( !TinyM_USI_TWI_Master_Start( ))
		{
			TinyM_USI_TWI_state.errorState = USI_TWI_BAD_MEM_READ;
			return (FALSE);                           // Send a START condition on the TWI bus.
		}
	  }
	  else
	  {
		TinyM_USI_TWI_state.addressMode = FALSE;            // Only perform address transmission once.
	  }
    }
    ...
    else
    {
      /* Read a data byte */
      DDR_USI   &= ~(1<<PIN_USI_SDA);               // Enable SDA as input.
      *(msg++)  = TinyM_USI_TWI_Master_Transfer( tempUSISR_8bit );
      ...
      TinyM_USI_TWI_Master_Transfer( tempUSISR_1bit );   // Generate ACK/NACK.
    }
...
---------------------------------------------------------------*/
unsigned char TinyM_USI_TWI_Master_Transfer( unsigned char temp )
{
...
/* Generate Start Condition */
  ...
  if( !(USISR & (1<<USISIF)) )
  {
    TinyM_USI_TWI_state.errorState = USI_TWI_MISSING_START_CON;  
    return (FALSE);
  }
#endif
  return (TRUE);
...
/*---------------------------------------------------------------
 ...
 the TWI bus.
---------------------------------------------------------------*/
unsigned char TinyM_USI_TWI_Master_Stop( void )
{
  ...
  if( !(USISR & (1<<USIPF)) )
  {
    TinyM_USI_TWI_state.errorState = USI_TWI_MISSING_STOP_CON;    
    return (FALSE);
  }
#endif
